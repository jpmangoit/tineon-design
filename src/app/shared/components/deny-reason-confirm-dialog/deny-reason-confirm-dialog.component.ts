	...
	deleteFunction() {
		if (this.data.section == 'News') {
			if (this.approved_status == 1) {
				this.authService.memberSendRequest('get', 'get-reset-updatednews/' + this.delete_id, null)
					.subscribe(
						(respData: any) => {
				...
				this.authService.memberSendRequest('delete', 'news/' + this.delete_id + '/user/' + userId, null)
					.subscribe(
						(respData: any) => {
							this.authService.setLoader(false);
		...
		if (this.data.section != 'News') {
			if (this.data.section == 'Group') {
				...
				if (this.approved_status == 0) {
					...
					this.navigation_path = this.deviceCheck + '/organizer/organizer-task';
				}
			} else if (this.data.section == 'Instructor') {
				if (this.approved_status == 1) {
					this.method = 'get';
