...
export class UpdateEventComponent implements OnInit, OnDestroy {
    ...
    group_dropdown: {
        id: any; group_id: number, name: string
    }[] = [];
    ...
    */
    setEventData() {
                    ...
                    if (groupList && groupList[0].length > 0) {
                        groupList[0].forEach((value: any, key: number) => {
                            if (value.id == this.eventDetails.group_id) {
                                ...
                                this.authService.memberSendRequest('get', 'approvedGroupUsers/group/' + value.id, null)
                                    .subscribe(
                                        (respData) => {
                                            this.authService.setLoader(false);
        ...
        if (type.length && visibility.length) {
            ...
            } else if (this.eventDetails.visibility == 1) {
        ...
        if (this.eventDetails && this.eventDetails.eventTask && Object.keys(this.eventDetails.eventTask).length != 0) {
            ...
            if (this.eventDetails?.eventTask?.group_id == 0) {
                ...
                if (self.group_dropdown && self.group_dropdown.length > 0) {
                    ...
                    this.authService.memberSendRequest('get', 'approvedGroupUsers/group/' + this.eventDetails?.eventTask?.group_id, null).subscribe(
                        (respData: any) => {
    ...
    eventProcess() {
        ...
        if ((this.eventForm.valid) && (!this.errorTime.isError) && (!this.errorDate.isError) && (!this.errorImage.isError)
            ...
            var date_to: string = this.eventForm.controls["date_to"].value;
